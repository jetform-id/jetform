defmodule AppWeb.AdminLive.Product.Components.Images do
    ...
    <div id={"images-for-product-" <> @id} class="p-4 md:p-8 dark:bg-gray-800 space-y-4">
      ...
      <div id="images-for-product" class="space-y-4" phx-update="stream">
        ...
        >
          <div class="flex items-start">
            ...
            <.button
              phx-click={JS.push("delete", value: %{id: image.id}, target: @myself)}
              type="button"
      ...
      <div :if={@upload_progress > 0}>
        <div class="flex justify-between pt-4 mb-2">
          <span class="text-sm font-medium text-slate-500">Uploading file...</span>
  ...
  def update(assigns, socket) do
    socket =
      ...
      |> stream(:images, Products.list_images(assigns.product))
      |> allow_upload(:image,
  ...
  defp handle_progress(:image, entry, socket) do
    if entry.done? do
      ...
      case Products.create_image(params) do
        {:ok, image} ->
          notify_parent()
          {:noreply, socket |> stream_insert(:images, image) |> set_progress(0)}

        {:error, changeset} ->
          {:noreply, socket |> assign(:changeset, changeset) |> set_progress(0)}
      end
    else
      {:noreply, set_progress(socket, entry.progress)}
    end
  end
